...
Pieces_on_board::Pieces_on_board() : board() {
...
void Pieces_on_board::kill_piece(std::shared_ptr<Piece> &_piece) {
	board.get_all_cells(_piece->get_position().x, _piece->get_position().y).free_cell();
	pob.remove(_piece);
}

void Pieces_on_board::pawn_replacement(std::shared_ptr<Piece> &_piece, int _select) {
...
int Pieces_on_board::select_piece_for_replacement(sf::RenderWindow &_window, std::shared_ptr<Piece> &_piece) {
	if (_piece->get_position().y == 7 || _piece->get_position().y == 0) {
...
std::list<std::shared_ptr<Piece>> &Pieces_on_board::get_pieces() {
	return pob;
}
Board& Pieces_on_board::get_board() {
	return board;
}

void Pieces_on_board::draw_for_white(sf::RenderWindow &_window) {
...
void Pieces_on_board::draw_for_black(sf::RenderWindow &_window) {
