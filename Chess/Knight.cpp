...
Knight::Knight(bool _color, bool _first_move, int _x, int _y) : Piece(_color, _first_move, _x, _y)
{
...
void Knight::possible_move(Board &_board) {
...
void Knight::change_position(int _x, int _y) {
	position.x = _x;
	position.y = _y;
	sprite.setPosition(coord_trans(position).x, coord_trans(position).y);
}

void Knight::draw_piece(sf::RenderWindow &_window) {
	_window.draw(sprite);
}