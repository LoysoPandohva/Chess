...
public:
	...
	void pawn_replacement(sf::RenderWindow &_window, std::shared_ptr<Piece> &_piece) {
		if (_piece->get_position().y == 7 || _piece->get_position().y == 0) {
			int select = select_piece_for_replacement(_window);
			switch (select) {
			case 1:
...
private:
	int select_piece_for_replacement(sf::RenderWindow &_window) {
